<?php
    ...
    switch (true) {
        ...
        case $uri === '/user_votes_control' && $method === 'POST':
            $in = getJson();
            $errs = validate($in,['id_user','id_rally','used_votes','id_photo']);
            if ($errs) {
                http_response_code(400);
                echo json_encode(['errors'=>$errs]);
                break;
            ...
            try {
                ...
                // 1. Obtener la configuración del rally
                $stmt = $pdo->prepare('
                    SELECT max_photos_user 
                    FROM configuration 
                    WHERE id_rally = :rally_id'
                );
                $stmt->execute([':rally_id' => $in['id_rally']]);
                $config = $stmt->fetch(PDO::FETCH_ASSOC);
                
                if (!$config) {
                    throw new Exception('No se encontró la configuración del rally');
                }

                // 2. Verificar votos usados por el usuario
                $stmt = $pdo->prepare('
                    SELECT COALESCE(SUM(used_votes), 0) as total_used
                    FROM user_votes_control 
                    WHERE id_user = :user_id 
                    AND id_rally = :rally_id'
                );
                $stmt->execute([
                    ':user_id' => $in['id_user'],
                    ':rally_id' => $in['id_rally']
                ]);
                $used = $stmt->fetchColumn();

                // Verificar si excede el límite
                if (($used + $in['used_votes']) > $config['max_photos_user']) {
                    $pdo->rollBack();
                    http_response_code(400);
                    echo json_encode(['error' => 'Has excedido el límite de votos permitidos']);
                    break;
                }
                ...
                // 4. Actualizar el total_votes de la fotografía
                $stmt = $pdo->prepare('
                    UPDATE photography 
